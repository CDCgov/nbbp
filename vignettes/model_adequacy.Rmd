...
We should therefore also simulated only datasets without singletons.
...
## Tails
One place we might expect our model to break down more is in the tails.
...
To examine this, let us return to the example of comparing $R$ for measles between the U.S. and Candada in the late 1990s.
...
ggplot(diff_predictive, aes(x = diff_means, y = after_stat(density))) +
  ...
  theme_minimal() +
  xlab("Absoluteifference in mean chain size") +
...
That the observed difference is unlikely confirms what we suspected from looking at the posteriors of $R$ from separate analyses: the pooled model is a poor fit, there is probably a difference.
