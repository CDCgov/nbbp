...
This is typically a subcritical ($R < 1$) regime, though with sufficient overconcentration (see below for the effect of $k$) is is possible in the supercritical regime.
...
The choice of a HalfNormal on $1 / \sqrt{k}$ follows best-practices for a ["weakly" informative prior](https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations#story-when-the-generic-prior-fails-the-case-of-the-negative-binomial) on the NegativeBinomial over-concentration parameter.

The parameter $k$ controls the concentration of the offspring distribution, as well as the final size distribution, and plays a strong role in the probability of extinction of a chain.
When $k$ is _large_, there is _less_ concentration relative to the mean, and in the limit of $k \to \infty$, the NegativeBinomial approaches the Poisson.
...
```{r, echo=FALSE, message=FALSE}
prior_predictive |>
  dplyr::mutate(p_0 = dnbinom(0, mu = r_eff, size = concentration)) |>
