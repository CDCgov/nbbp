...
Samples from the default prior are available in the package data object `prior_predictive`, which we will use to visualize the priors.
If one wishes to draw more samples, or explore alternative hyperparameters, modify the following code used to produce the pre-packaged data.


```{r, eval=FALSE, file="../data-raw/prior.R"}
```
...
The default prior on $R$ is Gamma(shape = 2.183089, rate = 2.183089).
