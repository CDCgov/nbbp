...
We discuss non-extinction in more depth in the "Advanced data" and "Details" vignettes.
...
Howeverm the variance is large, such that some infections may have a large number of offspring.
...
Aside from inference of $R$ and $k$ from final size data, `nbbp` implements R functions for computing the probability mass function (`dnbbp`), the cumlative distribution function (`pnbbp`), and random final size generation (`rnbbp`).
