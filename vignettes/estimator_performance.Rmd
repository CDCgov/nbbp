...
We can see that the two approaches have some similar behaviors, in that they both tend to underestimate $R$ when the true $R \approx 1$ and neither does amazingly well when the true $R = 2$ (Bayesian estimates are somewhat biased, maximum likelihood estimates very high-variance).
In general, unless $R$ is particularly large or small, it would seem that the posterior median is a better estimator than the maximum likelihood value, and as we will see later it handles better the small sample size case.
...
As discussed in the "Implementation details" vignette, the confidence intervals provided with maximum likelihood estimates use a hybrid approach, determining on a per-analysis basis whether to use parametric bootstrapping or likelihod profiling.
...
## $k$
Coverage of $k$ is also generally good for confidence and credible intervals.
Both confidence and credible intervals for $k$ perform better when $R$ is closer to 1, though credible intervals for are less sensitive to $R$ than confidence intervals.
...
Credible intervals are often much, much narrower than confidence intervals.
